...
export async function planQueries(
  ...
  return structured<QueryPlan>({
    system: [
      ...
      "- JOBSDB/JOBTHAI: short Thai/English keyword phrases a job board would match (these query the board's own search page, not a web search).",
...
export async function rankCandidates(
  ...
  return structured<RankResult>({
    system: [
      ...
      "NEVER drop a candidate from the output, no matter how poor the fit — wrong field, wrong seniority,",
      "sparse snippet with barely any info, all of that is still a KEEP. Score it honestly (a bad fit gets a",
      "low fitScore, e.g. under 20) and say plainly in concerns why it doesn't match or why there isn't enough",
      "evidence to judge — that's a valid, honest outcome for HR to see and decide on themselves. Every entry",
      "in the input must appear exactly once in the output shortlist. Order best-first.",
    ].join("\n"),
        ...
        shortlist: {
          ...
          items: {
            ...
            properties: {
              ...
              source: { type: "string", enum: ["LINKEDIN", "JOBSDB", "JOBBKK", "JOBTHAI", "FACEBOOK", "GITHUB", "WEB", "REFERRAL", "MANUAL"] },
